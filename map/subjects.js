        ...
        if (subject == "_global") {
            ...
            Object.keys(subject).forEach(key => {
                ...
                if (key.startsWith("script")) {
                    ...
                    let element = "head";
                    if(key.endsWith("body")) element = "body";
                    if (script.endsWith(".js")) {
                        $(element).append("<script src='" + script + "'><\/script>");
                    }
                    else if (script.startsWith("<script>") && script.endsWith("<\/script>")) {
                        $(element).append(script);
                    }
                    else {
                        $(element).append("<script>" + script + "<\/script>");
                    }
                }
            ...
            for (i = 0; i < Object.keys(subject).length; i++) {
                ...
                if (key.startsWith('content')) {
                    await load_html_template(key, subject[key]).then(content => {
